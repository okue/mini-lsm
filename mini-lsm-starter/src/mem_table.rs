...
impl MemTable {
    ...
    pub fn recover_from_wal(id: usize, path: impl AsRef<Path>) -> Result<Self> {
        ...
        for entry in skip_map.iter() {
            approximate_size += entry.key().raw_len();
            approximate_size += entry.value().len();
        }
        Ok(MemTable {
    ...
    pub fn scan(&self, lower: Bound<KeyBytes>, upper: Bound<KeyBytes>) -> MemTableIterator {
        let mut iter = MemTableIterator::new(
            ...
            (KeyBytes::default(), Bytes::default()),
        );
        iter.next().unwrap();
        iter
    }
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let entry = self
            .with_iter_mut(|iter| {
                let next = iter.next();
                next.map(|n| (n.key().clone(), n.value().clone()))
            })
            .unwrap_or((KeyBytes::default(), Bytes::default()));
